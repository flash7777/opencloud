...
func CS3ResourcePermissionsToLibregraphActions(p *provider.ResourcePermissions) []string {
	...
	if p.GetMoveContainer() {
		actions = append(actions, DriveItemContainerMove)
	}
	...
	if p.GetSetImmutableContainer() {
		actions = append(actions, DriveItemImmutableContainerSet)
	}
